...
export function PaywallDialog({ open, onOpenChange, productSlug, sourceRoute, inputPayload }: PaywallDialogProps) {
  ...
  const total = product.priceHuf + (express && product.category === "delayed" ? EXPRESS_PRICE_HUF : 0);
          ...
          <div className="space-y-4">
            <div className="text-center">
              ...
              <div className="text-xs text-ivory/55 mt-1">
                {product.category === "instant" ? "azonnali olvasat" : `kiszállítás emailben ${express ? EXPRESS_HOURS + " órán" : "12–24 órán"} belül`}
              </div>
            </div>

            <div>
              <label className="block text-sm text-ivory/80 mb-1">Email cím (ide küldjük)</label>
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="te@pelda.hu"
              ...
              {!user && <p className="text-xs text-ivory/45 mt-1">Vendég vásárlás — fiók nem kötelező.</p>}
            </div>
            ...
            {product.category === "delayed" && (
              <label className="flex items-start gap-3 p-3 rounded-md border border-[oklch(0.78_0.10_80/0.2)] cursor-pointer">
                ...
                <div>
                  <div className="text-ivory">Express — 6 órán belül <span className="text-gold">+{formatHuf(EXPRESS_PRICE_HUF)}</span></div>
            ...
            <p className="text-[10px] text-ivory/40 text-center">Biztonságos Stripe fizetés. Bankkártya, Apple Pay, Google Pay.</p>
          </div>
        ) : (
