...
export const Route = createFileRoute("/harom-lap")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Klasszikus három lapos tarot húzás magyarul. Múlt, jelen, jövő — egy összefüggő történet." },
    ],
...
const CATEGORIES = [
  "szerelem", "randi / ismerkedés", "ex / visszatérés", "munka",
  "pénz", "család", "döntés előtt", "általános élethelyzet",
];
...
function HaromLap() {
  ...
  return (
    <Layout>
      <PageHeader eyebrow="Három lap" title="Múlt · Jelen · Jövő" lead="Három lap, egy ív. Nem külön-külön, hanem együtt mond valamit." />
      <div className="mx-auto max-w-5xl px-4 md:px-6 pb-20 space-y-8">
        {!cards && (
          <form onSubmit={(e) => { e.preventDefault(); draw(); }} className="surface p-6 md:p-7 space-y-5 max-w-2xl mx-auto">
            <div>
              <label className="block text-sm text-ivory/80 mb-2">Mire kérdezel? <span className="text-ivory/50">(opcionális)</span></label>
              <input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Pl. Most lépjek-e tovább ebben?"
            ...
            <button type="submit" className="btn-gold w-full md:w-auto">Húzom a három lapot</button>
          </form>
        )}
        ...
        {cards && (
          <>
            <div className="grid grid-cols-3 gap-3 md:gap-6 max-w-3xl mx-auto">
              {cards.map((c, i) => (
                <div key={i}>
                  <div className="text-center text-[10px] tracking-[0.3em] uppercase text-[oklch(0.78_0.10_80/0.7)] mb-2">{LABELS[i]}</div>
                  {revealed[i]
                    ? <CardFace card={c} />
                    : <button onClick={() => setRevealed((r) => r.map((v, j) => j === i ? true : v))} className="block w-full"><CardBack /></button>}
                </div>
              ))}
            </div>
            ...
            <div className="text-center pt-4">
              <button onClick={() => setCards(null)} className="btn-ghost-gold">Új húzás</button>
            </div>
            {question && <p className="text-center text-ivory/50 text-sm font-editorial italic">A kérdésed: „{question}" — kategória: {category}</p>}
          </>
        )}
      </div>
    </Layout>
