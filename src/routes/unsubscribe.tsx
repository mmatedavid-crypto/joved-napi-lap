    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md w-full rounded-2xl border border-border bg-card p-8 text-center shadow-sm">
        ...
        <div className="mt-6 text-sm text-foreground">
          ...
          {status === 'error' && (
            <p>Valami megakadt. Kérlek próbáld újra később.</p>
          )}
          {status === 'valid' && (
            <>
              <p>
                Biztosan leiratkozol a Jövőd.hu emailjeiről? Ezután nem küldünk
                több üzenetet erre a címre.
              </p>
              <button
                onClick={confirm}
                disabled={submitting}
          ...
          {status === 'done' && (
            <p>Sikeresen leiratkoztál. Köszönjük, hogy velünk voltál.</p>
          )}
        </div>
      </div>
    </main>
