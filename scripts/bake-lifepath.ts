...
${JSON.stringify(raw).slice(0, 4000)}`;
  ...
  try {
    ...
    console.error(
      "Failed to parse Hungarian rendering output (first 500 chars):",
      content.slice(0, 500),
    );
...
function applyCuratedLifePathOverrides(out: Record<string, unknown>) {
  const items = out as Record<string, Record<string, string>>;
  if (items["6"]) {
    items["6"] = {
      ...items["6"],
      shadow:
...
async function main() {
  ...
  applyCuratedLifePathOverrides(out);

  const body = `// AUTO-GENERATED by scripts/bake-lifepath.ts — do not edit by hand.
// Source: curated life-path source material, rendered as Hungarian symbolic copy.
